...
mkValidator dat () ctx = traceIfFalse "invalid beneficiary signature" signedByBeneficiary &&
    ...
    where
      ...
      validDeadline :: Bool
      validDeadline = ((txSignedBy info $ beneficiary1 dat) &&
                      (txInfoValidRange info `overlaps` (to $ deadline dat))) ||
                      ((txSignedBy info $ beneficiary2 dat) && 
                      (txInfoValidRange info `overlaps` (from $ deadline dat + 1)))
