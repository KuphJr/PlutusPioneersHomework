...
mkValidator keyHash deadline () ctx = traceIfFalse "beneficiary's signature missing" signedByBeneficiary &&
  ...
  where
    ...
    signedByBeneficiary = txSignedBy info $ keyHash
...
typedValidator keyHash deadline = Scripts.mkTypedValidator @Vesting
    ($$(PlutusTx.compile [|| mkValidator ||]) `PlutusTx.applyCode` PlutusTx.liftCode keyHash `PlutusTx.applyCode` PlutusTx.liftCode deadline)
